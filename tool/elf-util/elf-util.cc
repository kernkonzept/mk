           ...
           addr_t base_addr)
{
  ...
  addr_t ram_max_addr = 0;
  addr_t ram_min_addr = ~0ull;
  addr_t align = 0;
  addr_t xip_addr = ~0ULL;
      ...
      if (phdr->p_flags & PF_X)
        {
          ...
          if (xip_addr == ~0ULL)
            xip_addr = phdr->p_paddr - phdr->p_offset;
          else if (xip_addr != phdr->p_paddr - phdr->p_offset)
            xip_addr = ~1ULL;
        }

      if (phdr->p_flags & PF_W)
        {
          if (ram_min_addr > phdr->p_paddr)
            ram_min_addr = phdr->p_paddr;
          if (ram_max_addr < e)
            ram_max_addr = e;
        }
  ...
  printf("RAM_ADDR_MIN_%d=0x%llx\n", idx, trunc_align(ram_min_addr, align) - base_addr);
  printf("RAM_ADDR_MAX_%d=0x%llx\n", idx, round_align(ram_max_addr, align) - base_addr);
  printf("ALIGNMENT_%d=0x%llx\n", idx, align);
  if (xip_addr != ~0ULL && xip_addr != ~1ULL)
    printf("XIP_ADDR_%d=0x%llx\n", idx, xip_addr);
