...
static void replace_button_icon(GtkWidget *widget, const char *filename)
{
	...
	gchar *path = g_strconcat(env ? env : g_get_current_dir(), "/tool/kconfig/scripts/kconfig/icons/", filename, NULL);
...
static void init_right_tree(void)
{
	...
	char *env = getenv(SRCTREE);
	gchar *path = g_strconcat(env ? env : g_get_current_dir(), "/tool/kconfig/scripts/kconfig/icons/menu.xpm", NULL);
	GError *err = NULL;
...
int main(int ac, char *av[])
{
	...
	env = getenv(SRCTREE);
	if (env)
		glade_file = g_strconcat(env, "/tool/kconfig/scripts/kconfig/gconf.ui", NULL);
	else if (av[0][0] == '/')
